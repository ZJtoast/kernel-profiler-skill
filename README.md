...
5. Run the staged collector, extract compact metrics, generate source hotspots, and generate the visual report if requested.
6. Write the final artifacts under `./profile/<kernel_name>_<profile_id>/`.

Manual usage is still supported: `profile-target.yaml` can be generated by hand from `assets/templates/profile-target.yaml`, edited after generation, or bypassed by calling the scripts directly.
...
The standard collector runs profiling in evidence-oriented stages:
...
Some profiler counters require elevated privileges. Three execution modes are supported:

| Mode | Purpose |
|---|---|
| `none` | Run without sudo. |
| `authorized_sudo` | Run directly only when root, cached sudo, or narrow `NOPASSWD` sudoers access is already configured. |
| `manual_sudo_script` | Generate a shell script that is run once with `sudo bash ...`. |

Plaintext password storage is not implemented. Passwords must not be written into YAML, scripts, logs, environment variables, shell history, generated commands, or reports.
...
scripts/create_sudo_profile_handoff.py        manual sudo handoff script generator
...
| Field | Meaning |
|---|---|
| `mode` | `none`, `authorized_sudo`, or `manual_sudo_script`. |
| `authorized_sudo_policy` | Restricts direct sudo to root, cached sudo, or narrow `NOPASSWD`. |
| `generate_manual_sudo_handoff` | Allows generating `run_profile_with_sudo.sh`. |
| `handoff_script_name` | Name of the manual sudo handoff script. |
| `allow_internal_sudo_per_command` | Must remain `false`; the handoff script is run as one sudo process. |
| `password_storage` | Must remain `forbidden`. |
| `forbidden` | Explicit safety rules for passwords and privilege scope. |
...
When non-interactive sudo is not available, generate a handoff script:
...
Run it as one privileged script:

```bash
sudo bash ./profile/hgemm_byzj_v0_20260515_120000/run_profile_with_sudo.sh
```

The script is intended to run as a single sudo process, not as a sequence of internal `sudo ncu ...` calls.
...
Stable kernel-only profiling package with native CUDA and Python/Triton support. The package includes target generation, staged Nsight Compute collection, optional visual reports, optional before/after regression comparison, optional bottleneck decision rules, source hotspot extraction, sudo handoff support, NVIDIA hardware metadata, vendor-portable metric aliases, and backend conformance checks.
...
Defined the final privilege model: no-sudo execution, authorized sudo execution, and manual sudo handoff. Kernel-name requests now generate a direct regex filter by default, with discovery only as a fallback.

### v0.4

Added manual sudo handoff script generation for environments where interactive sudo cannot be used by the profiler process. Refined README tone and clarified how privileged collection should be run.
